...
def evaluate(
    ...
    for ind, image_info in tqdm(filenames.iterrows()):
        ...
        #prediction = cv.imread(f'{os.path.join(predictions_path, name)}.png')[:,:,0]
        #prediction = ((prediction / 255) > threshold)*255
        ...
        #labels, distance = watershed_segmentation(mask)
        ...
        test_df_results = test_df_results.append({'dataset_folder': image_info['dataset_folder'] , 'name': name, 'position': image_info['position'],
                                'img_height': img_size[1], 'img_width': img_size[0],
